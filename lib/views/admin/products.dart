...
import 'package:cafito/bd/image.dart';
import '../../bd/firebase_services.dart';
import '../../const.dart';
...
class _ProductState extends State<Product> {
  ...
  final TextEditingController _presentationproductController = TextEditingController();
            ...
            actions: [
              TextButton(
                  ...
                  if (image_to_upload != null) {
                    imageUrl = await uploadImageToFirebaseStorage(image_to_upload!);
                  }
                ...
                children: [
                  ...
                  TextButton(
                    onPressed: () async {
                      ...
                      String imageUrl;
                      if (selectedLocalImage != null) {
                        imageUrl = await uploadImageToFirebaseStorage(selectedLocalImage!);
                      } else {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Producto editado con éxito')),
                      );
                      _updateProductList();
...
void _addToInventory({required Map<String, dynamic> productData, required String productUid}) {
          ...
          TextButton(
            onPressed: () async {
              ...
              if (existingInventoryDoc.docs.isNotEmpty) {
                ...
                final existingQuantity = existingInventoryDoc.docs.first['cantidad'];
                ...
                await existingInventoryDoc.docs.first.reference.update({
                 ...
                 if (newQuantity <= 0) {
                  String inventoryDocumentUid = existingInventoryDoc.docs.first.id;
              ...
              Navigator.pop(context); // Cierra el formulario después de agregar al inventario
            },
              ...
              if(snapshot.hasData){
                ...
                return Dismissible(
                  ...
                  onDismissed: (direction) async {
                    showDialog(
                      context: context,
                      barrierDismissible: false, // Evita que el diálogo se cierre tocando fuera de él
                      builder: (BuildContext context) {
                        return const Center(
                          child: CircularProgressIndicator(),
                        );
                    ...
                    return await showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          ...
                          content: Text('¿Estás seguro de que deseas eliminar este producto: ${snapshot.data?[index]['name']}?'),
                          actions: <Widget>[
                            TextButton(
                  ...
                  child: ListTile(
                    leading: ClipRRect(
                      ...
                      child: snapshot.data?[index]['image'] == ''
                          ? Container(child: const Icon(Icons.image), width: 60, height: 60) // Show an icon if no image URL is found
                          : Image.network(
                              snapshot.data?[index]['image'], // Use the image URL from the data
                              width: 60,
                              height: 60,
                              fit: BoxFit.cover,
                            ) 
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            _addToInventory(productData: snapshot.data![index], productUid: snapshot.data?[index]['uid']);
                          },
                          icon: const Tooltip(
                            message: 'Enviar',
                            child: Icon(Icons.send_and_archive_outlined, color: Colors.green,),
                          ),
                        ),
